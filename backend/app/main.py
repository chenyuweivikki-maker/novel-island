...
from .services.guidance import (
    is_creative_query as _is_creative_query,
    is_new_book_intent as _is_new_book_intent,
    empty_kb_stream_or_dict as _empty_kb_stream_or_dict,
)
...
def _build_outline_context(novel_id: int) -> str:
...
@app.post("/api/novel/{novel_id}/outline/generate")
def generate_novel_outline(novel_id: int):
    """一键生成全文大纲：从项目知识（人物/事件/章纲/背景）用 LLM 生成 5 块，写入库并返回"""
    ctx = _build_outline_context(novel_id)
    try:
        out = chat(OUTLINE_GEN_PROMPT.format(context=ctx), "请根据资料生成结构化大纲。",
                   temperature=0.6, max_tokens=700, task="summary")
    except Exception as e:
        print(f"[outline] 生成失败: {e}")
        return {"success": False, "error": "大纲生成失败，请稍后再试"}
    sections = _parse_outline_json(out)
    if not sections:
