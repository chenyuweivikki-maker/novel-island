...
ENTITY_EXTRACT_PROMPT = """你是「小说岛」的文本分析专家。请阅读下面的小说片段，抽取**人物**实体及属性。

规则：
1. 只抽取原文**逐字出现**的**人物**；名字必须在输入文本里找到字面。严禁由题材/角色数量/剧情联想补充输入里没有的名字。
2. **关键：区分实体类型**，每个实体都要标注 type：
   - `person`：人物（角色、人名，如 江观南）
   - `place`：地名（城市/街道/地区名，如 望城、上海）——**不是人物**
   - `book`：书名/作品名（如 观南嘉措）——**不是人物**
   - `object`：物品/其他非人实体
   **书名、地名、物品一律不要作为人物（type=person）输出**；它们如果对剧情重要可带 type 输出，但 type 必须是 place/book/object，绝不能是 person。
3. 每个人物记录：name、type、identity、attributes（职业/年龄/外貌/性格/家庭/经历/创伤/动机/物品/宠物/事件，原文有才写，没有留空）。
4. 输出严格的 JSON 数组，格式：
[{"name": "角色名", "type": "person", "identity": "身份", "attributes": {"职业": "...", "年龄": "...", "外貌": "...", "性格": "...", "家庭": "...", "经历": "...", "创伤": "...", "动机": "...", "物品": "...", "宠物": "...", "事件": "..."}}]
...
class EntityExtractNode:
    ...
    def __call__(self, state: NovelIslandState) -> Dict[str, Any]:
        ...
        # 硬过滤 1b：只保留"人物"，排除书名/地名/物品（type=place/book/object/other 不作为人物实体）
        entities = [e for e in entities if e.get("type", "person") == "person"]
